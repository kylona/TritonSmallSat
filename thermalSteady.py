...
scat_minP = 100 #min scaterometry power lost to heat
radiometry_maxP = 10 #max radiometry power lost to heat
radiometry_minP = 3 #min radiometry power lost to heat
percentScatNeeded = 7/100.0 #percent of time we need to run scaterometry
...
def componentHeatMax(temp):
  global percentScatTime
  if(temp > 80): #if we are getting too hot turn off scatterometry
    percentScatTime = max(0, percentScatNeeded - (temp-70)/20) 
  else:
    percentScatTime = percentScatNeeded
...
def componentHeatMin(temp):
  global percentScatTime
  if(temp < 20): #if we are getting too cold turn on scatterometry to produce heat
    percentScatTime =  min(1,percentScatNeeded + (15-temp)/20) 
  else:
    percentScatTime = percentScatNeeded 
...
for sunlightPercent in range(minSunlightPercent,maxSunlightPercent + 1, 10):
  ...
  while True:
     ...
     deltaJules = heatIn
     deltaJules += componantHeatMax(maxT)
  ...
  print("Max Temperature: {} (found in {} steps) set Scatterometry time to {}%".format(maxT, numSteps, percentScatTime))
  ...
  while True:
     ...
     #compute net change in jules
     deltaJules = heatIn
     deltaJules += componantHeatMin(minT)
     ...
     if abs(deltaJules) < juleEpsilon: break
  print("Min Temperature: {} (found in {} steps) set Scaterometry time to {}%".format(minT, numSteps, percentScatTime))
