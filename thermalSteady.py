...
def componantHeatMax(temp):
  if(temp > 80): #if we are getting too cold turn on scatterometry to produce heat
    deltaJules = scat_maxP * max(0, percentScatTime - (temp-70)/20) + radiometry_maxP * percentOperating_max + componantP
  else:
...
def componantHeatMin(temp):
  if(temp < 20): #if we are getting too cold turn on scatterometry to produce heat
    deltaJules = scat_maxP * min(1,(percentScatTime + (20-temp)/20)) + radiometry_minP *   min(1,(percentOperating_min + (20-temp)/100.0)) + componantP
  else:
